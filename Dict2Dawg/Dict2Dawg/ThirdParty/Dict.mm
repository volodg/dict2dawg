...
static void eachEdge( const Edge& edge_ )
{
...
//   NSMutableArray* array_ = [ NSMutableArray arrayWithCapacity: edges.size() ];
   ...
   std::string nums_ = std::string();
   for ( NSUInteger index_ = 0; index_ < edges.size(); ++index_ )
   {
      str_.push_back( edges[index_].val + 65 );
      NSString* num_str_ = [ NSString stringWithFormat: @", %d", edges[index_].ptr ];
      nums_.append( [ num_str_ cStringUsingEncoding: NSASCIIStringEncoding ] );
   }
...
//		CCLOG(@"Node::serialize: isTerm read error.");
   ...
   NSLog( @"Node: %@ %@", [ [ NSString alloc ] initWithBytes: str_.c_str() length: str_.length() encoding: NSASCIIStringEncoding ]
         , isTerm ? @"+":@"-" );
   NSLog( @"Node n: %@", [ [ NSString alloc ] initWithBytes: nums_.c_str() length: nums_.length() encoding: NSASCIIStringEncoding ] );
}
