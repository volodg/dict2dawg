...
#define TRADITIONAL_DAWG_DATA "/Users/vgor/Traditional_Dawg_For_Word-List.dat"
#define TRADITIONAL_DAWG_TEXT_DATA "/Users/vgor/Traditional_Dawg_Explicit_Text_For_Word-List.txt"
...
static void CutOffExtraChars(char *ThisLine){
...
static int StringToPositiveInt(char* TheNumberNotYet){
	int Result = 0;
	int X;
...
static void ConvertIntNodeToBinaryString(int TheNode, char *BinaryNode){
	int X;
	int Bit;
	BinaryNode[0] = '[';
...
static void MakeMeAllCapital(char *RawWord){
...
static int TnodeArrayIndex(TnodePtr ThisTnode){
	return ThisTnode->ArrayIndex;
}

static char TnodeDirectChild(TnodePtr ThisTnode){
	return ThisTnode->DirectChild;
}

static TnodePtr TnodeNext(TnodePtr ThisTnode){
	return ThisTnode->Next;
}

static TnodePtr TnodeChild(TnodePtr ThisTnode){
	return ThisTnode->Child;
}

static TnodePtr TnodeParentalUnit(TnodePtr ThisTnode){
	return ThisTnode->ParentalUnit;
}

static TnodePtr TnodeReplaceMeWith(TnodePtr ThisTnode){
	return ThisTnode->ReplaceMeWith;
}

static char TnodeLetter(TnodePtr ThisTnode){
	return ThisTnode->Letter;
}

static char TnodeMaxChildDepth(TnodePtr ThisTnode){
	return ThisTnode->MaxChildDepth;
}

static char TnodeNumberOfChildren(TnodePtr ThisTnode){
	return ThisTnode->NumberOfChildren;
}

static char TnodeEndOfWordFlag(TnodePtr ThisTnode){
	return ThisTnode->EndOfWordFlag;
}

static char TnodeLevel(TnodePtr ThisTnode){
	return ThisTnode->Level;
}

static char TnodeDangling(TnodePtr ThisTnode){
	return ThisTnode->Dangling;
}

static char TnodeProtected(TnodePtr ThisTnode){
	return ThisTnode->Protected;
}

// Allocate a "Tnode" and fill it with initial values.
static TnodePtr TnodeInit(char Chap, TnodePtr OverOne, char WordEnding, char Leveler, unsigned long StarterDepth, TnodePtr Parent, char IsaChild){
...
static void TnodeSetArrayIndex(TnodePtr ThisTnode, int TheWhat){
	ThisTnode->ArrayIndex = TheWhat;
}

static void TnodeSetChild(TnodePtr ThisTnode, TnodePtr Chump){
	ThisTnode->Child = Chump;
}

static void TnodeSetNext(TnodePtr ThisTnode, TnodePtr Nexis){
	ThisTnode->Next = Nexis;
}

static void TnodeSetParentalUnit(TnodePtr ThisTnode, TnodePtr Parent){
	ThisTnode->ParentalUnit = Parent;
}

static void TnodeSetReplaceMeWith(TnodePtr ThisTnode, TnodePtr Living){
	ThisTnode->ReplaceMeWith = Living;
}

static void TnodeSetMaxChildDepth(TnodePtr ThisTnode, unsigned long NewDepth){
	ThisTnode->MaxChildDepth = NewDepth;
}

static void TnodeSetDirectChild(TnodePtr ThisTnode, char Status){
	ThisTnode->DirectChild = Status;
}

static void TnodeFlyEndOfWordFlag(TnodePtr ThisTnode){
...
static int TnodeDangle(TnodePtr ThisTnode){
...
static void TnodeProtect(TnodePtr ThisTnode){
	ThisTnode->Protected = TRUE;
}

// This function removes "Protected" status from "ThisNode".
static void TnodeUnProtect(TnodePtr ThisTnode){
...
static Bool TnodeProtectionCheck(TnodePtr ThisTnode, Bool CheckThisTnode){
...
static TnodePtr TnodeFindParaNode(TnodePtr ThisTnode, char ThisLetter){
...
static void TnodeInsertParaNode(TnodePtr AboveTnode, char ThisLetter, char WordEnder, unsigned long StartDepth){
...
static char TnodeAreWeTheSame(TnodePtr FirstNode, TnodePtr SecondNode){
...
static TnodePtr DawgRootNode(DawgPtr ThisDawg){
...
static int TnodeDawgAddWord(TnodePtr ParentNode, const char *Word){
	int Result = 0;
	int X, Y = 0;
...
static void DawgAddWord(DawgPtr ThisDawg, char * NewWord){
...
static void TnodeGraphTabulateRecurse(TnodePtr ThisTnode, int Level, int* Tabulator){
...
static void DawgGraphTabulate(DawgPtr ThisDawg, int* Count){
	int Numbers[MAX_MAX];
	int X = 0;
	for ( X = 0; X < MAX_MAX; X++ ) Numbers[X] = 0;
...
static void TnodeLawnMowerRecurse(TnodePtr ThisTnode){
...
static void DawgLawnMower(DawgPtr ThisDawg){
...
static Bool TnodeProtectAndReplaceBranchStructure(TnodePtr EliminateThis, TnodePtr ReplaceWith, Bool ValidReplacement){
...
static void BreadthQueueNodeSetNext(BreadthQueueNodePtr ThisBreadthQueueNode, BreadthQueueNodePtr Nexit){
   ThisBreadthQueueNode->Next = Nexit;
}

static BreadthQueueNodePtr BreadthQueueNodeNext(BreadthQueueNodePtr ThisBreadthQueueNode){
   return ThisBreadthQueueNode->Next;
}

static TnodePtr BreadthQueueNodeElement(BreadthQueueNodePtr ThisBreadthQueueNode){
   return ThisBreadthQueueNode->Element;
}

static BreadthQueueNodePtr BreadthQueueNodeInit(TnodePtr NewElement){
...
static BreadthQueuePtr BreadthQueueInit(void){
...
static void BreadthQueuePush(BreadthQueuePtr ThisBreadthQueue, TnodePtr NewElemental){
...
static TnodePtr BreadthQueuePop(BreadthQueuePtr ThisBreadthQueue){
...
static void BreadthQueuePopulateReductionArray(BreadthQueuePtr ThisBreadthQueue, TnodePtr Root, TnodePtr **Holder){
	int Taker[MAX_MAX];
	memset(Taker, 0, MAX_MAX*sizeof(int));
...
static int BreadthQueueUseToIndex(BreadthQueuePtr ThisBreadthQueue, TnodePtr Root){
	int IndexNow = 0;
	TnodePtr Current = Root;
	// Push the first row onto the queue.
	while ( Current != NULL ) {
...
static void ArrayDnodeInit(ArrayDnodePtr ThisArrayDnode, char Chap, int Nextt, int Childd, char EndingFlag, char Breadth){
...
static void ArrayDnodeTnodeTranspose(ArrayDnodePtr ThisArrayDnode, TnodePtr ThisTnode){
...
static int ArrayDnodeNext(ArrayDnodePtr ThisArrayDnode){
	return ThisArrayDnode->Next;
}

static int ArrayDnodeChild (ArrayDnodePtr ThisArrayDnode){
	return ThisArrayDnode->Child;
}

static char ArrayDnodeLetter(ArrayDnodePtr ThisArrayDnode){
	return ThisArrayDnode->Letter;
}

static char ArrayDnodeEndOfWordFlag(ArrayDnodePtr ThisArrayDnode){
...
void write2binary( int NumberOfLivingNodes, ArrayDawgPtr Result );
void write2text( int NumberOfLivingNodes, ArrayDawgPtr Result );
...
ArrayDawgPtr ArrayDawgInit(char **Dictionary, int *SegmentLenghts, int MaxStringLength){
	...
	for ( X = 0; X < MAX_MAX; X++ ) HolderOfAllTnodePointers[X] = (TnodePtr *)malloc(NodeNumberCounterInit[X]*sizeof(TnodePtr));
   ...
   //Step 13
	write2binary( NumberOfLivingNodes, Result );
   
   //Step 14
   write2text( NumberOfLivingNodes, Result );
...
//int dawg_converter( char* inFileNameWithDict );
int dict2dawg_converter( const char* inFileNameWithDict ){
	...
	int X;
	// All of the words of similar length will be stored sequentially in the same array so that there will be (MAX_MAX + 1)  arrays in total.
	// The Smallest length of a string is assumed to be 2.
	char *AllWordsInEnglish[MAX_MAX + 1];
	for ( X = 0; X < (MAX_MAX + 1); X++ ) AllWordsInEnglish[X] = NULL;
	
	FILE *Input;
	Input = fopen(inFileNameWithDict,"r");
	char ThisLine[100] = "\0";
   //	int FirstLineIsSize;
	size_t LineLength;
   //	
   ...
   //   int FirstLineIsSize = 267750;///Users/vgor/sowpods.txt
	...
	for ( X = 0; X < FirstLineIsSize; X++ ) {
		...
		if ( LineLength <= MAX_MAX ) DictionarySizeIndex[LineLength] += 1;
	...
	for ( X = 2; X < (MAX_MAX + 1); X++ ) AllWordsInEnglish[X] = (char *)malloc((X + 1)*DictionarySizeIndex[X]*sizeof(char));
	
	int CurrentTracker[MAX_MAX + 1];
	for ( X = 0; X < (MAX_MAX + 1); X++ ) CurrentTracker[X] = 0;
	...
	for ( X = 0; X < FirstLineIsSize; X++ ) {
		...
		if ( CurrentLength <= MAX_MAX ) strcpy( &((AllWordsInEnglish[CurrentLength])[(CurrentTracker[CurrentLength]*(CurrentLength + 1))]),
                                             LexiconInRam[X] );
	...
	//ArrayDawgPtr Adoggy = 
   ArrayDawgInit(AllWordsInEnglish, DictionarySizeIndex, MAX_MAX);
